...
class TestMLKEM(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.encaps_expected_lookup = {
            ...
            for test in cls.encaps_decaps_expected_data["testGroups"][0]["tests"]
        }
        
        # Create lookup for decapsulation test data
        cls.decaps_expected_lookup = {
            test["tcId"]: bytes.fromhex(test["k"])
            for test in cls.encaps_decaps_expected_data["testGroups"][3]["tests"]
        }
        
    def test_keygen(self):
        print("\nRunning key generation tests...")
        for test in self.keygen_prompt_data["testGroups"][0]["tests"]:
            tc_id = test["tcId"]
    ...
    def test_encaps(self):
        ...
        for test in self.encaps_decaps_prompt_data["testGroups"][0]["tests"]:
            tc_id = test["tcId"]
    ...
    def test_decaps(self):
        ...
        dk = bytes.fromhex(self.encaps_decaps_prompt_data["testGroups"][3]["dk"])
        
        for test in self.encaps_decaps_prompt_data["testGroups"][3]["tests"]:
            tc_id = test["tcId"]
